...
public class TelaEditarUsuario extends javax.swing.JFrame {
    ...
    public TelaEditarUsuario(String cpfUsuarioLista) throws SQLException {
        initComponents();
        this.cpf = cpfUsuarioLista;
    ...
    private void buscarInformacoes() {
        String sql = "SELECT nome, telefone, cpf, cep, rua, bairro, cidade, estado, numero, senha FROM usuario WHERE cpf = ?";

        try (Connection conexao = conexaoBanco.getConnection(); PreparedStatement stmt = conexao.prepareStatement(sql)) {
...
private void editarUsuario() {
    ...
    if (nome.isEmpty() || telefone.isEmpty() || senha.isEmpty() || cep.isEmpty()
            || rua.isEmpty() || bairro.isEmpty() || cidade.isEmpty() || estado.isEmpty() || numero.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Por favor, preencha todos os campos.");
        return;
    }

    String sql = "UPDATE usuario SET nome = ?, telefone = ?, senha = ?, cep = ?, rua = ?, bairro = ?, cidade = ?, estado = ?, numero = ? WHERE cpf = ?";

    ConexaoBancoDeDados conexaoBanco = new ConexaoBancoDeDados();
    Connection conexao = conexaoBanco.getConnection(); 

    if (conexao == null) {
        JOptionPane.showMessageDialog(this, "Erro: Conexão com o banco de dados não pode ser estabelecida.");
        return;
    }
    ...
    try (PreparedStatement stmt = conexao.prepareStatement(sql)) {
        ...
        if (linhasAfetadas > 0) {
            ...
            JOptionPane.showMessageDialog(this, "Erro ao editar perfil: usuário não encontrado.");
        }
    } catch (SQLException ex) {
        JOptionPane.showMessageDialog(this, "Erro ao editar perfil: " + ex.getMessage());
        ex.printStackTrace();
    } finally {
        conexaoBanco.close(conexao); 
    }
}
